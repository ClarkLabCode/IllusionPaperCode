function [ f ] = MakeModelFilters_T5( p )
...
% Spatial High Pass Version
% sMexE = p.fwhmMexicanE / (2*sqrt(2*log(2)));
% sMexI = p.fwhmMexicanI / (2*sqrt(2*log(2)));
% 
% f.spatialFilter = exp(-(p.x - p.xExtent/2).^2/(2*sMexE^2)) / sqrt(2 * pi * sMexE^2) * p.dx...
%                  -exp(-(p.x - p.xExtent/2).^2/(2*sMexI^2)) / sqrt(2 * pi * sMexI^2) * p.dx;
