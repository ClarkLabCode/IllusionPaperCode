function [meanResp, voltageResp, calciumResp, meanNumResp, meanDenResp, meanNumRespLN ] = ComputeThreeInputModelResponse_T5(stimArray, p, f)
...
if isinf(p.inputRectBeta)
    relu = @(f)(f .* (f>0));
    relu_Tm1 = @(f)((f-th(1)) .* (f>th(1)));
    relu_CT1 = @(f)((f-th(2)) .* (f>th(2)));
else
...
% Input 1 ~ Tm9 off inhibitory
% Input 2 ~ Tm1/Tm2/Tm4 off excitatory
% Input 3 ~ ? off inhibitory
g1 = p.g1 .* relu(-resp1);
g2 = p.g2 .* relu_Tm1(-resp2);
g3 = p.g3 .* relu_CT1(-resp3);
