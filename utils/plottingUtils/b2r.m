function newmap = b2r(cmin_input,cmax_input,center_val, color_top_mid_bottom, color_num)
...
if nargin >5
   ...
   disp('or input three variables, where the last is center which is white, for example: colormap(b2r(-3, 3, 0));')
   disp('or input four variables, where the last is a 3x3 matrix whose first row is the max color, middle is the mid color, and last row is the min color;' )
   disp('or input five variables, where the last is the number of colors in the colormap');
end

if nargin<2
    figH = gcf;
    img = figH.findobj('Type', 'Image');
    imgDat = img.CData;
    cmin_input = min(imgDat(:));
    cmax_input = max(imgDat(:));
    color_top_mid_bottom = [0.4039         0    0.1216;
                            1 1 1;
...
red_top     = color_top_mid_bottom(1, :);
white_middle= color_top_mid_bottom(2, :);
blue_bottom = color_top_mid_bottom(3, :);
